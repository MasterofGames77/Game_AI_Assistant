  ...
  try {
    ...
    if (response.data.length > 0) {
      const games = response.data.map((game: IGDBGame) => ({
        ...
        genres: game.genres?.map((genre: { name: string }) => genre.name).join(', '),
        platforms: game.platforms?.map((platform: { name: string }) => platform.name).join(', '),
...
const fetchDataFromBothAPIs = async (gameName: string): Promise<string> => {
  try {
    const [rawgResponse, igdbResponse] = await Promise.all([fetchGamesFromRAWG(gameName), fetchGamesFromIGDB(gameName)]);

    let finalResponse = "Combined Game Information:\n";

    if (!rawgResponse.includes("No games found")) finalResponse += `From RAWG: ${rawgResponse}\n`;

    if (!igdbResponse?.includes("No games found")) finalResponse += `From IGDB: ${igdbResponse}`;
...
const checkAndUpdateGameInfo = async (question: string, answer: string): Promise<string> => {
  ...
  try {
    ...
    return gameInfo ? `${formatGameInfo(gameInfo)}\n\nAdditional Information:\n${combinedResponse}` : `${answer}\n\nAdditional Information:\n${combinedResponse}`;
