...
import { downloadCardImage, CardData } from "../utils/cardGenerator";
import { extractSnippet, extractGameTitle } from "../utils/snippetExtractor";
...
const ShareCardModal: React.FC<ShareCardModalProps> = ({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const filename = `${cardData.gameTitle.replace(
        /[^a-z0-9]/gi,
        "_"
      )}_${Date.now()}.png`;
  ...
  const handleShareToDiscord = async () => {
    ...
    try {
      ...
      // Discord doesn't support direct image sharing via URL, so we'll:
      // 1. Copy to clipboard if supported
      // 2. Otherwise, download the image
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          ...
          {/* Debug: Show card data */}
          {cardData && !previewUrl && (
            <div className="mb-4 p-4 bg-gray-100 dark:bg-gray-700 rounded text-sm">
              <p>
                <strong>Game:</strong> {cardData.gameTitle}
              </p>
              <p>
                <strong>Question:</strong> {cardData.question.substring(0, 50)}
                ...
              </p>
              <p>
                <strong>Snippet:</strong>{" "}
                {cardData.answerSnippet.substring(0, 50)}...
              </p>
            </div>
          )}
